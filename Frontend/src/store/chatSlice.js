...
const chatSlice = createSlice({
    ...
    reducers: {
        ...
        deleteChat(state, action) {
            ...
            state.chats = state.chats.filter(chat => chat.id !== idToDelete && chat._id !== idToDelete);
            // If the deleted chat was active, unset or select another chat
            if (state.activeChatId === idToDelete) {
                state.activeChatId = state.chats.length ? state.chats[0].id || state.chats[0]._id : null;
            }
        },
